...
def list(request, initial_mode="login"):
    ...
    noticeList = Repository.objects.order_by('-id')
    ...
    return render_to_response('repository/list.html', context)
...
def detail(request, initial_mode="login"):
    ...
    notice = get_object_or_404(Repository, id=noticeId)
    ...
    return render_to_response('repository/detail.html', context)
...
def checkPopup(request):
    ...
    #serialized_obj = serializers.serialize('json', [ noticeList, ])
    #noticeId = request.GET.get('noticeId');
    #serialized = json.dumps(notice, ensure_ascii=False)
    #serialized = serializers.serialize('json', noticeList)
    #serialized = {'test': 'test'}
    #serialized = {}
    #serialized['detail_url']='naver.com'
    #serialized['contents']= ''
    #serialized['id']= ''
    #serailized['title']= ''
    #response = json.dumps(serialized, ensure_ascii=True).decode('utf8')
    #serialized = {'title': '한글'}
